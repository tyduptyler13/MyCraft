...
THREE.Sky=function(a){if(void 0===a||null===a)a=5E3;var c=THREE.ShaderLib.sky,b=THREE.UniformsUtils.clone(c.uniforms),c=new THREE.ShaderMaterial({fragmentShader:c.fragmentShader,vertexShader:c.vertexShader,uniforms:b,side:THREE.BackSide});a=new THREE.SphereBufferGeometry(a,32,15);this.mesh=new THREE.Mesh(a,c);this.uniforms=b};(function(){var a={},c={},b=new THREE.TextureLoader,d={},e=new THREE.MultiMaterial,f=e.visible=!1,m=[];API.getMaterial=function(g,e){a[g]?e(a[g]):async.each(["diffuseMap","normalMap","specularMap"],function(a,e){d[g][a]?b.load(d[g][a],function(b){c[g]||(c[g]={});c[g][a]=b;b.wrapS=b.wrapT=THREE.RepeatWrapping;e()},function(){},function(){e(Error("Texture failed to load."))}):e()},function(b){if(b)throw b;b=new THREE.MeshPhongMaterial({map:c[g].diffuseMap,specularMap:c[g].specularMap,normalMap:c[g].normalMap,
name:d[g].name});a[g]=b;e(b)})};$.ajax({url:"blocks.json",success:function(a){d=a;var c=[];async.forEachOf(a,function(a,b,d){0<=b?API.getMaterial(b,function(a){d();c[b]=a}):d()},function(a){a&&console.error("Unable to prefetch the required textures.");e.materials=c;f=e.visible=!0;m.forEach(function(a){try{a()}catch(c){console.warn("Failed to call callback!",c)}})})},error:function(a,c,b){console.log(a,c,b)}});API.getTexture=function(a,b){return c[a][b]};API.getTextures=function(){return c};API.setAnisotropy=
function(a){for(var b in c)for(var d in c[b]){var e=c[b][d];e.anisotropy=a;e.needsUpdate=!0}};API.getBlockMaterial=function(){return e};API.onBlocksReady=function(a){f?a():m.push(a)}})();
var chunkPool=new ThreadPool("js/ChunkOptimizer.min.js"),Chunk=function(a){this.blocks=new Int8Array(512);this.space=new THREE.Object3D;this.added=!1;var c=this.geometry=new THREE.BufferGeometry;this.attributes={position:new THREE.BufferAttribute(new Float32Array,3),uv:new THREE.BufferAttribute(new Float32Array,2),normal:new THREE.BufferAttribute(new Float32Array,3)};c.addAttribute("position",this.attributes.position);c.addAttribute("uv",this.attributes.uv);c.addAttribute("normal",this.attributes.normal);
...
!1;break;case 39:case 68:b.right=!1;break;case 32:b.jump=!1;break;case 16:b.crouch=!1}});var d=this,e=Math.PI/2;$(document).mousemove(function(a){if(!0===c){var b=a.originalEvent.movementY;d.player.rotation.y-=.002*a.originalEvent.movementX;d.head.rotation.x-=.0015*b;d.head.rotation.x=Math.max(-e,Math.min(e,d.head.rotation.x))}});var f=function(){var b=new THREE.Mesh(new THREE.BoxGeometry(1,1,1),new THREE.MeshBasicMaterial({color:"white",fog:!1,wireframe:!0}));b.scale.multiplyScalar(1.01);a.scene.add(b);
var c=new THREE.Raycaster;c.far=10;var d=new THREE.Vector2,e=new THREE.Vector3;return function(){c.setFromCamera(d,a.camera);var f=c.intersectObjects(a.scene.children,!0);if(0!==f.length)for(var l=0,n=f.length;l<n;++l)if(f[l].object.userData.chunk){e.subVectors(f[l].point,f[l].face.normal);f=f[l].face.normal;0<f.x+f.y+f.z?b.position.copy(e.floor().addScalar(.5)):b.position.copy(e.ceil().subScalar(.5));b.visible=!0;return}b.visible=!1}}();a.tasks.push(function(a){c&&(d.move(b,a),f())})};
...
MyCraft.prototype.setupSky=function(a,c){var b={luminance:1,inclination:.49,azimuth:.3};b.inclination=Date.now()/18E5%2;var d=new THREE.Mesh(new THREE.SphereBufferGeometry(5,16,8),new THREE.MeshBasicMaterial({color:16777215}));d.frustumCulled=!1;d.visible=!1;d.fog=!1;var e=new THREE.DirectionalLight(16777215,1);e.position.copy(d.position);e.castShadow=!0;e.shadowCameraNear=1;e.shadowCameraFar=132;e.shadowCameraLeft=-32;e.shadowCameraRight=32;e.shadowCameraTop=32;e.shadowCameraBottom=-32;e.shadowMapWidth=
2048;e.shadowMapHeight=2048;e.target=this.player.mesh;var f=new THREE.Sky(a);f.mesh.frustumCulled=!1;f.mesh.add(d);f.mesh.add(e);f.mesh.position.copy(c.position);this.scene.add(f.mesh);f.mesh.fog=!1;this.scene.fog=new THREE.FogExp2(0,.02);for(var m=new THREE.Object3D,g=new THREE.Geometry,k=.95*a,h=0;1E4>h;++h){var p=new THREE.Vector3(Math.random()-.5,Math.random()-.5,Math.random()-.5);p.normalize().multiplyScalar(k);g.vertices.push(p)}var l=new THREE.PointsMaterial({size:1,blending:THREE.AdditiveBlending,
sizeAttenuation:!1,color:16777215,fog:!1,transparent:!0}),g=new THREE.Points(g,l);g.frustumCulled=!1;m.add(g);f.mesh.add(m);var n=f.uniforms;n.distance.value=a;var q=function(b){n.luminance.value=b.luminance;var c=Math.PI*(b.inclination-.5);b=2*Math.PI*(b.azimuth-.5);d.position.x=a*Math.cos(b);d.position.y=a*Math.sin(b)*Math.sin(c);d.position.z=a*Math.sin(b)*Math.cos(c);f.uniforms.sunPosition.value.copy(d.position)};q(b);var r=new THREE.Color(6216),t=(new THREE.Color(16777215)).multiplyScalar(.5),
u=this;this.tasks.push(function(){b.inclination=Date.now()/18E5%2;l.opacity=Math.pow(1-Math.abs(1-b.inclination),2);e.intensity=THREE.Math.clamp(d.position.y+1,0,1);0<d.position.y?u.ambientLight.color.copy(r.clone().lerp(t,d.position.y/100)):u.ambientLight.color.copy(t.clone().lerp(r,-d.position.y/100));q(b);e.position.copy(d.position);f.mesh.position.copy(c.position);m.lookAt(d.position)})};
MyCraft.prototype.setupChunks=function(){for(var a=this,c=function(b,c){for(var d=new Chunk(Math.floor(9*Math.random()-2)),g=Math.floor(8*Math.random()),k=0;8>k;++k)for(var h=0;8>h;++h)d.set(k,g,h,-1);k=Math.floor(8*Math.random());for(g=0;8>g;++g)for(h=0;8>h;++h)d.set(k,g,h,-1);h=Math.floor(8*Math.random());for(g=0;8>g;++g)for(k=0;8>k;++k)d.set(k,g,h,-1);d.position.set(b,-8,c);a.scene.add(d.space);a.chunks[d.position.toArray().join(",")]=d;d.update()},b=-10;10>b;++b)for(var d=-10;10>d;++d)async.nextTick(c.bind(null,
8*b,8*d))};
