...
THREE.Sky=function(a){if(void 0===a||null===a)a=5E3;var c=THREE.ShaderLib.sky,b=THREE.UniformsUtils.clone(c.uniforms),c=new THREE.ShaderMaterial({fragmentShader:c.fragmentShader,vertexShader:c.vertexShader,uniforms:b,side:THREE.BackSide});a=new THREE.SphereBufferGeometry(a,32,15);this.mesh=new THREE.Mesh(a,c);this.uniforms=b};(function(){var a={},c={},b=new THREE.TextureLoader,d={},e=new THREE.MultiMaterial,g=e.visible=!1,l=[];API.getMaterial=function(f,e){a[f]?e(a[f]):async.each(["diffuseMap","normalMap","specularMap"],function(a,e){d[f][a]?b.load(d[f][a],function(b){c[f]||(c[f]={});c[f][a]=b;b.wrapS=b.wrapT=THREE.RepeatWrapping;e()},function(){},function(){e(Error("Texture failed to load."))}):e()},function(b){if(b)throw b;b=new THREE.MeshPhongMaterial({map:c[f].diffuseMap,specularMap:c[f].specularMap,normalMap:c[f].normalMap,
name:d[f].name});a[f]=b;e(b)})};$.ajax({url:"blocks.json",success:function(a){d=a;var c=[];async.forEachOf(a,function(a,b,d){0<=b?API.getMaterial(b,function(a){d();c[b]=a}):d()},function(a){a&&console.error("Unable to prefetch the required textures.");e.materials=c;g=e.visible=!0;l.forEach(function(a){try{a()}catch(c){console.warn("Failed to call callback!",c)}})})},error:function(a,c,b){console.log(a,c,b)}});API.getTexture=function(a,b){return c[a][b]};API.getTextures=function(){return c};API.setAnisotropy=
function(a){for(var b in c)for(var d in c[b]){var e=c[b][d];e.anisotropy=a;e.needsUpdate=!0}};API.getBlockMaterial=function(){return e};API.onBlocksReady=function(a){g?a():l.push(a)}})();
var chunkPool=new ThreadPool("js/ChunkOptimizer.min.js"),Chunk=function(a){this.blocks=new Int8Array(512);this.space=new THREE.Object3D;this.added=!1;var c=this.geometry=new THREE.BufferGeometry;c.frustumCulled=!1;this.attributes={position:new THREE.BufferAttribute(new Float32Array,3),uv:new THREE.BufferAttribute(new Float32Array,2),normal:new THREE.BufferAttribute(new Float32Array,3)};c.addAttribute("position",this.attributes.position);c.addAttribute("uv",this.attributes.uv);c.addAttribute("normal",
this.attributes.normal);this.mesh=new THREE.Mesh(c,API.getBlockMaterial());this._metaBlocks=[];0!==a&&this.fill(a)};Chunk.prototype.at=function(a,c,b){return this.blocks[a+8*c+64*b]};Chunk.getPos=function(a){return[a&7,a>>3&7,a>>6&7]};Chunk.prototype.set=function(a,c,b,d){a=a+8*c+64*b;if(0>a||511<a)throw Error("Out of bounds.");this.blocks[a]=d};Chunk.prototype.fill=function(a){this.blocks.fill(a)};
Chunk.prototype.walk=function(a){for(var c=0,b=0;8>b;++b)for(var d=0;8>d;++d)for(var e=0;8>e;++e)a(this.blocks[c],e,d,b),c++};
Chunk.prototype.update=function(){var a=this;chunkPool.run(this.blocks.slice(0),function(c){c=c.data;a.attributes.position.array=new Float32Array(c.position);a.attributes.position.needsUpdate=!0;a.attributes.normal.array=new Float32Array(c.normals);a.attributes.normal.needsUpdate=!0;a.attributes.uv.array=new Float32Array(c.uvs);a.attributes.uv.needsUpdate=!0;c=new Uint8Array(c.materialIndex);a.geometry.clearGroups();for(var b=0;b<c.length;++b)a.geometry.addGroup(108*b,108,c[b]);a.added||(API.onBlocksReady(function(){a.space.add(a.mesh)}),
...
MyCraft.prototype.setupSky=function(a,c){var b={turbidity:10,reileigh:2,mieCoefficient:.005,mieDirectionalG:.8,luminance:1,inclination:.49,azimuth:.3};b.inclination=Date.now()/18E5%2;var d=new THREE.Mesh(new THREE.SphereBufferGeometry(2E4,16,8),new THREE.MeshBasicMaterial({color:16777215}));d.visible=!1;d.fog=!1;var e=new THREE.DirectionalLight(16777215,1);e.position.copy(d.position);e.position.normalize();this.scene.add(e);var g=new THREE.Sky(a);g.mesh.frustumCulled=!1;g.mesh.add(d);g.mesh.position.copy(c.position);
this.scene.add(g.mesh);g.mesh.fog=!1;this.scene.fog=new THREE.FogExp2(0,.0025);for(var l=new THREE.Object3D,f=new THREE.Geometry,k=.9*a,h=0;1E4>h;++h){var n=new THREE.Vector3(Math.random()-.5,Math.random()-.5,Math.random()-.5);n.normalize().multiplyScalar(k);f.vertices.push(n)}var p=new THREE.PointsMaterial({size:1,blending:THREE.AdditiveBlending,sizeAttenuation:!1,color:16777215,fog:!1,transparent:!0}),f=new THREE.Points(f,p);f.frustumCulled=!1;l.add(f);l.position.copy(c.position);this.scene.add(l);
var m=g.uniforms;m.distance.value=a;var q=function(b){m.turbidity.value=b.turbidity;m.reileigh.value=b.reileigh;m.luminance.value=b.luminance;m.mieCoefficient.value=b.mieCoefficient;m.mieDirectionalG.value=b.mieDirectionalG;var c=Math.PI*(b.inclination-.5);b=2*Math.PI*(b.azimuth-.5);d.position.x=a*Math.cos(b);d.position.y=a*Math.sin(b)*Math.sin(c);d.position.z=a*Math.sin(b)*Math.cos(c);g.uniforms.sunPosition.value.copy(d.position)};q(b);var r=new THREE.Color(6216),t=(new THREE.Color(16777215)).multiplyScalar(.7);
this.tasks.push(function(){b.inclination=Date.now()/18E5%2;p.opacity=Math.pow(1-Math.abs(1-b.inclination),2);e.intensity=THREE.Math.clamp(d.position.y+1,0,1);0<d.position.y?this.ambientLight.color.copy(r.clone().lerp(t,d.position.y/1E4+.5)):this.ambientLight.color.copy(t.clone().lerp(r,-d.position.y/1E4+.5));q(b);e.position.copy(d.position);e.position.normalize();l.position.copy(c.position);l.lookAt(d.position);g.mesh.position.copy(c.position)})};
MyCraft.prototype.setupChunks=function(){for(var a=this,c=function(b,c){for(var d=new Chunk(Math.floor(8*Math.random()-2)),f=Math.floor(8*Math.random()),k=0;8>k;++k)for(var h=0;8>h;++h)d.set(k,f,h,-1);k=Math.floor(8*Math.random());for(f=0;8>f;++f)for(h=0;8>h;++h)d.set(k,f,h,-1);h=Math.floor(8*Math.random());for(f=0;8>f;++f)for(k=0;8>k;++k)d.set(k,f,h,-1);d.position.set(b,-8,c);a.scene.add(d.space);a.chunks[d.position.toArray().join(",")]=d;d.update()},b=-10;10>b;++b)for(var d=-10;10>d;++d)async.nextTick(c.bind(null,
8*b,8*d))};
