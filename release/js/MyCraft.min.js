...
2048;e.shadowMapHeight=2048;e.target=this.player.mesh;var f=new THREE.Sky(a);f.mesh.frustumCulled=!1;f.mesh.add(d);f.mesh.add(e);f.mesh.position.copy(c.position);this.scene.add(f.mesh);f.mesh.fog=!1;this.scene.fog=new THREE.FogExp2(0,.02);for(var l=new THREE.Object3D,g=new THREE.Geometry,k=.95*a,h=0;1E4>h;++h){var m=new THREE.Vector3(Math.random()-.5,Math.random()-.5,Math.random()-.5);m.normalize().multiplyScalar(k);g.vertices.push(m)}var n=new THREE.PointsMaterial({size:1,blending:THREE.AdditiveBlending,
sizeAttenuation:!1,color:16777215,fog:!1,transparent:!0}),g=new THREE.Points(g,n);g.frustumCulled=!1;l.add(g);f.mesh.add(l);var p=f.uniforms;p.distance.value=a;var q=function(b){p.luminance.value=b.luminance;var c=Math.PI*(b.inclination-.5);b=2*Math.PI*(b.azimuth-.5);d.position.x=a*Math.cos(b);d.position.y=a*Math.sin(b)*Math.sin(c);d.position.z=a*Math.sin(b)*Math.cos(c);f.uniforms.sunPosition.value.copy(d.position)};q(b);var r=new THREE.Color(6216),t=(new THREE.Color(16777215)).multiplyScalar(.5),
u=this;this.tasks.push(function(){b.inclination=Date.now()/18E5%2;n.opacity=Math.pow(1-Math.abs(1-b.inclination),2);e.intensity=THREE.Math.clamp(d.position.y+1,0,1);0<d.position.y?u.ambientLight.color.copy(r.clone().lerp(t,d.position.y/100)):u.ambientLight.color.copy(t.clone().lerp(r,-d.position.y/100));q(b);e.position.copy(d.position);f.mesh.position.copy(c.position);l.lookAt(d.position)})};
