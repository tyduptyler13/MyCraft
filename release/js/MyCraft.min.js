...
THREE.Sky=function(a){if(void 0===a||null===a)a=5E3;var c=THREE.ShaderLib.sky,b=THREE.UniformsUtils.clone(c.uniforms),c=new THREE.ShaderMaterial({fragmentShader:c.fragmentShader,vertexShader:c.vertexShader,uniforms:b,side:THREE.BackSide});a=new THREE.SphereBufferGeometry(a,32,15);this.mesh=new THREE.Mesh(a,c);this.uniforms=b};(function(){var a={},c={},b=new THREE.TextureLoader,d={},e=new THREE.MultiMaterial,f=e.visible=!1,l=[];API.getMaterial=function(e,f){a[e]?f(a[e]):async.each(["diffuseMap","normalMap","specularMap"],function(a,f){d[e][a]?b.load(d[e][a],function(b){c[e]||(c[e]={});c[e][a]=b;b.wrapS=b.wrapT=THREE.RepeatWrapping;f()},function(){},function(){f(Error("Texture failed to load."))}):f()},function(b){if(b)throw b;b=new THREE.MeshPhongMaterial({map:c[e].diffuseMap,specularMap:c[e].specularMap,normalMap:c[e].normalMap,
name:d[e].name});a[e]=b;f(b)})};$.ajax({url:"blocks.json",success:function(a){d=a;var c=[];async.forEachOf(a,function(a,b,d){0<=b?API.getMaterial(b,function(a){d();c[b]=a}):d()},function(a){a&&console.error("Unable to prefetch the required textures.");e.materials=c;f=e.visible=!0;l.forEach(function(a){try{a()}catch(c){console.warn("Failed to call callback!",c)}})})},error:function(a,c,b){console.log(a,c,b)}});API.getTexture=function(a,b){return c[a][b]};API.getTextures=function(){return c};API.setAnisotropy=
function(a){for(var b in c)for(var d in c[b]){var e=c[b][d];e.anisotropy=a;e.needsUpdate=!0}};API.getBlockMaterial=function(){return e};API.onBlocksReady=function(a){f?a():l.push(a)}})();
...
MyCraft.prototype.setupSky=function(a,c){var b={turbidity:10,reileigh:2,mieCoefficient:.005,mieDirectionalG:.8,luminance:1,inclination:.49,azimuth:.3};b.inclination=Date.now()/18E5%2;var d=new THREE.Mesh(new THREE.SphereBufferGeometry(5,16,8),new THREE.MeshBasicMaterial({color:16777215}));d.frustumCulled=!1;d.visible=!1;d.fog=!1;var e=new THREE.DirectionalLight(16777215,1);e.position.copy(d.position);e.castShadow=!0;e.shadowCameraNear=1;e.shadowCameraFar=132;e.shadowCameraLeft=-32;e.shadowCameraRight=
32;e.shadowCameraTop=32;e.shadowCameraBottom=-32;e.shadowMapWidth=2048;e.shadowMapHeight=2048;e.target=this.player.mesh;var f=new THREE.Sky(a);f.mesh.frustumCulled=!1;f.mesh.add(d);f.mesh.add(e);f.mesh.position.copy(c.position);this.scene.add(f.mesh);f.mesh.fog=!1;this.scene.fog=new THREE.FogExp2(0,.02);for(var l=new THREE.Object3D,g=new THREE.Geometry,k=.95*a,h=0;1E4>h;++h){var n=new THREE.Vector3(Math.random()-.5,Math.random()-.5,Math.random()-.5);n.normalize().multiplyScalar(k);g.vertices.push(n)}var p=
new THREE.PointsMaterial({size:1,blending:THREE.AdditiveBlending,sizeAttenuation:!1,color:16777215,fog:!1,transparent:!0}),g=new THREE.Points(g,p);g.frustumCulled=!1;l.add(g);f.mesh.add(l);var m=f.uniforms;m.distance.value=a;var q=function(b){m.turbidity.value=b.turbidity;m.reileigh.value=b.reileigh;m.luminance.value=b.luminance;m.mieCoefficient.value=b.mieCoefficient;m.mieDirectionalG.value=b.mieDirectionalG;var c=Math.PI*(b.inclination-.5);b=2*Math.PI*(b.azimuth-.5);d.position.x=a*Math.cos(b);d.position.y=
a*Math.sin(b)*Math.sin(c);d.position.z=a*Math.sin(b)*Math.cos(c);f.uniforms.sunPosition.value.copy(d.position)};q(b);var r=new THREE.Color(6216),t=(new THREE.Color(16777215)).multiplyScalar(.5),u=this;this.tasks.push(function(){b.inclination=Date.now()/18E5%2;p.opacity=Math.pow(1-Math.abs(1-b.inclination),2);e.intensity=THREE.Math.clamp(d.position.y+1,0,1);0<d.position.y?u.ambientLight.color.copy(r.clone().lerp(t,d.position.y/100)):u.ambientLight.color.copy(t.clone().lerp(r,-d.position.y/100));q(b);
e.position.copy(d.position);f.mesh.position.copy(c.position);l.lookAt(d.position)})};
